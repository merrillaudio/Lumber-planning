...
def generate_csv(cut_plan):
    ...
    for board in cut_plan:
        board_project = board['board'].get('project', '')
        for cut in board['cuts']:
            piece_project = cut['piece'].get('project', board_project)
            writer.writerow([
                board['board_id'],
                piece_project,
                cut['piece']['id'],
...
def generate_pdf(cut_plan, leftovers=None, job_title=""):
    ...
    with PdfPages(buffer) as pdf:
        if job_title:
            cover_fig, ax = plt.subplots(figsize=(8.5, 11))
            ax.axis('off')
            ax.text(0.5, 0.5, job_title, fontsize=24, ha='center', va='center')
        ...
        if leftovers:
            ...
            for piece in leftovers:
                lines.append(f"{to_fraction_string(piece['length']):>8} {to_fraction_string(piece['width']):>8}")
